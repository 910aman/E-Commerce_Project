...
const Categories = () => {
  ...
  return (
    <>
      <div className="bg-gray-100 mx-auto px-4 text-xs cursor-pointer gap-x-3 gap-y-1 flex flex-wrap overflow-auto py-2">
      ...
      <div className='text-gray-900 text-xl py-1 px-4 bg-gray-100'>
         <Heading heading={`See our ${selectedFilters.length === 0 ? "All" : selectedFilters} Collection`} />
        <div className="grid grid-cols-1 gap-x-2 gap-y-3 sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4  xl:gap-x-8">
            ...
            <div key={index} className="group relative cursor-pointer bg-gray-200 hover:bg-[#cbd5e1] hover:rounded-e-lg hover:rounded-s-lg rounded-s-lg rounded-e-lg"  >
              <Link to={`/products/${product?.id}`} state={product}>
                ...
                <div className="px-3 py-2 justify-between">
                  <p className="pb-1 text-sm flex justify-center border-b-[0.5px]  border-gray-500 text-gray-700">Brand: {product?.brand}</p>
                  ...
                  <div className='flex justify-center gap-2'>
                    ...
                    <p className="flex items-center text-lg font-medium pr-2 border-r-2 border-gray-800 text-gray-700">${((product?.price) - (product?.price * (product?.discountPercentage / 100))).toFixed(2)}</p>
